...
class AdPostValidator:
    ...
    def validate(cls, title: str, content: str, tags: List[str] = None) -> Tuple[bool, List[str], Dict]:
        ...
        if char_count_no_space < 1200:
            issues.append(f"[글자 수 미달] 공백 제외 최소 1,200자 이상이어야 하지만 현재 {char_count_no_space}자입니다.")
        ...
        if not has_qa:
            issues.append("[정보성 보완 권장] Q&A나 핵심 체크리스트 단락이 포함되면 승인률이 올라갑니다.")

        # 치명적 이슈(글자 수, 금지어, 제목)가 없으면 합격 판정
        critical_issues = [i for i in issues if "[글자 수 미달]" in i or "[승인 감점" in i or "[제목 길이" in i]
...
if __name__ == "__main__":
    sample_title = "2026 알뜰교통카드 K-패스 신청 방법 및 환급 혜택 총정리"
    sample_content = """## 1. K-패스란 무엇인가요?\n\n대중교통을 이용할 때 환급 혜택을 주는 카드입니다. """ * 80
    sample_tags = ["#K패스", "#교통카드", "#생활정보"]
    
    passed, logs, st = AdPostValidator.validate(sample_title, sample_content, sample_tags)
    print("검증 통과 여부:", passed)
    print("통계:", st)
    for l in logs:
        print("-", l)
